...
SampledSpectrum PathIntegrator::Li(RayDifferential ray, SampledWavelengths &lambda,
    ...
    while (true) {
       ...
       //std::cout<<"Sampleld"<<std::endl;
        if (IsNonSpecular(bsdf.Flags())) {
            ++totalPaths;
            SampledSpectrum Ld = SampleLd(isect, &bsdf, lambda, sampler, sampleLd_);
            sampleLd_ = false;
            if (!Ld)
                ++zeroRadiancePaths;
            L += beta * Ld;
        ...
        Float u = sampler.Get1D();
        pstd::optional<BSDFSample> bs;
       //std::cout<<"Samplebsdf"<<std::endl;
        if(!sampleLd_) bs = bsdf.Sample_f(wo, u, sampler.GetPixel2D());
        else bs = bsdf.Sample_f(wo, u, sampler.Get2D());
...
SampledSpectrum PathIntegrator::SampleLd(const SurfaceInteraction &intr, const BSDF *bsdf,
                                         ...
                                         Sampler sampler, bool intersect) const {
...
SampledSpectrum VolPathIntegrator::Li(RayDifferential ray, SampledWavelengths &lambda,
    ...
    while (true) {
                    ...
                    if (mode == 0) {
                        ...
                        if (beta && r_u) {
                            ...
                            MediumInteraction intr(p, -ray.d, ray.time, ray.medium,
                                                   mp.phase);
                           //std::cout<<"SampleLd scatter"<<std::endl;
                            L += SampleLd(intr, nullptr, lambda, sampler, beta, r_u, false);

                            // Sample new direction at real-scattering event
                           //std::cout<<"Sample new dir scatter"<<std::endl;
                            Point2f u = sampler.Get2D();
                            pstd::optional<PhaseFunctionSample> ps =
       ...
       //std::cout<<"\tsampleLd attenuated"<<std::endl;
        if (IsNonSpecular(bsdf.Flags())) {
            L += SampleLd(isect, &bsdf, lambda, sampler, beta, r_u, sampleLd_);
            sampleLd_ = false;
...
SampledSpectrum VolPathIntegrator::SampleLd(const Interaction &intr, const BSDF *bsdf,
                                            SampledWavelengths &lambda, Sampler sampler,
                                            SampledSpectrum beta,
                                            SampledSpectrum r_p, bool intersect) const {
